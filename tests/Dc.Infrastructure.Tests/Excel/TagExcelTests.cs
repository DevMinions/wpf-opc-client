...
public class TagExcelTests
{
    ...
    public void WriteAndRead_Roundtrip()
    {
        ...
        var tags = new[]
        {
            new Tag { Id = "id1", Item = "Random.Int1", DataType = 2, TaskId = "task-1" },
            new Tag { Id = "id2", Item = "Random.Real8", DataType = 5, TaskId = "task-1" },
            new Tag { Id = "id3", Item = "中文.Item", DataType = 8, TaskId = "task-2" }
        };

        using var ms = new MemoryStream();
        service.Write(tags, ms);
        ms.Position = 0;
        ...
        Assert.Equal(8, rows[2].DataType);
    }
    ...
    public void Read_SkipsEmptyItemRows()
    {
        ...
        var tags = new[]
        {
            new Tag { Id = "id1", Item = "A", DataType = 1, TaskId = "t" }
        };
        using var ms = new MemoryStream();
        service.Write(tags, ms);
        ms.Position = 0;
